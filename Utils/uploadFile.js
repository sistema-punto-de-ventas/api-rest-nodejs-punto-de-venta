...
const AWS = require('./upload_file_aws/uploadFileAWS');
...
const uploadFileFotoProducto = (req, res) => {
    ...
    upload(req, res, async (err) => {
        try {
            ...
            console.log(req.file) */
            if (!req.file) return res.status(206).send({ status: 'No fount', message: 'La imagen es obligatorio' })
            /* console.log(req.params.idmenu , ' esto es el id');
            console.log(req.file, 'esto es la imgen'); */
            if (err) return res.status(206).send({ status: 'No fount', message: 'error al guardar el archivo' });

            const ruta = req.file.path.substr(6);
            const restaurant = await producto.findById({ _id: req.params.idmenu });
            if (restaurant) {
                ...
                newFile.save(async (err, data) => {
                    ...
                    //var fotoProduct = await AWS.UplaodFileAWS(data.physicalPath, data.nameFile);

                    let arr = []
                     arr.push({ img: ruta })
                    //arr.push({ img: fotoProduct })
                    await producto.findOneAndUpdate({ _id: req.params.idmenu }, { urlImages: arr })
                    const newRest = await producto.findById({ _id: req.params.idmenu })
            ...
            console.log(error);
            res.status(400).json({ status: 'No fount', message: 'Error al guardar la imagen', error })
        }
